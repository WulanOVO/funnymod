...
public class ModNetworking {
    public static void initialize() {
        ...
        PayloadTypeRegistry.serverboundPlay().register(SkeletonHorseShootPayload.TYPE, SkeletonHorseShootPayload.CODEC);
        ...
        // 服务器接收器：定位骷髅马并发射弩箭（回调运行在服务器主线程）
        ServerPlayNetworking.registerGlobalReceiver(SkeletonHorseShootPayload.TYPE, (payload, context) -> {
            Entity entity = context.player().level().getEntity(payload.horseId());
            if (entity instanceof FireworkDashHorse horse && context.player().getVehicle() == entity) {
                horse.funnymod$shootCrossbow(context.player());
            }
        });
    }
}
